...
if [ "$(whoami)" = "root" ]; then
    ...
    exit 1
        ...
        # installing
        sudo apt install xorg-xrandr i3 feh fish polybar rofi alacritty picom
        ...
        /usr/bin/cp -r /usr/share/rofi /home/$user/.config/
        ...
        sudo /usr/bin/cp ./picom.conf /etc/xdg/picom.conf

        # permissions directory scripts
        /usr/bin/chown $user:$user /home/$user/.config/polybar/script
        /usr/bin/chown $user:$user /home/$user/.config/polybar/script/*.sh

        # zsh config file
        sudo /usr/bin/cp .zshrc /home/$user/.zshrc
        sudo /usr/bin/cp -r zshrc.pre-oh-my-zsh /home/$user/.zshrc.pre-oh-my-zsh
        change_shell
        ...
        /usr/bin/echo "THANKS FOR USE MY RICE! RESTART COMPUTER OR NOTEBOOK AND SELECT I3"
